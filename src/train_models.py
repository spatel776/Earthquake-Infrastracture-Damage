...
parser.add_argument('--name', default='Final_EID_4_CLASS_ResNet18', type=str, help='name of the run')
parser.add_argument('--seed', default=42, type=int, help='random seed')
parser.add_argument('--num-class', default=4, type=int, help='Number of class in the classification')

# For load EID dataset
parser.add_argument('--train', default='./data/labels/label_final_train.tsv', type=str, help='path to train image files/labels')
parser.add_argument('--dev', default='./data/labels/label_final_val.tsv', type=str, help='path to dev image files/labels')
parser.add_argument('--test', default='./data/labels/label_final_test.tsv', type=str, help='path to test image files/labels')

parser.add_argument('--out-file', default='out/Final/Final_EID_4_CLASS_ResNet18/Final_EID_4_CLASS_ResNet18.json', type=str, help='path to output file')
...
parser.add_argument('--fig-dir', default='out/figures/Final/Final_EID_4_CLASS_ResNet18', type=str, help='directory path for output figures')
...
parser.add_argument('--use-rand-augment', default=True, type=lambda x: (str(x).lower() == 'true'),
...
parser.add_argument('--three-class', default=False, type=lambda x: (str(x).lower() == 'true'),
                    help='use in 3 class classification test. merge none and irrelevant')
    ...
    for epoch in range(curr_epoch, num_epochs):
        ...
        for phase in phases:
            ...
            if phase == 'val':
                ...
                #if epoch % 50 == 0:
                #    print("save state_dict for epoch {}".format(epoch))
...
def main():
    ...
    # save the arg information in fig_dir
    arg_dir = fig_dir + "/Config.json"
    training_dir = fig_dir + "/training_output.txt"

    # specify the output file
    output_file = open(training_dir, 'w')
    ...
    # now every print call will go to the file specified
    print("This will be written to the file.")


    #save the arg information in fig_dir
    #arg_dir = fig_dir + ".json"
    ...
    if args.use_rand_augment:
        data_transforms['train'].transforms.insert(0, torchvision.transforms.RandAugment(args.rand_augment_n, args.rand_augment_m))
    ...
    image_datasets = {
                'train': dataset.Final_dataset(train_path,  sep, data_dir, data_transforms['train'],three_class=three_class),
                'val': dataset.Final_dataset(dev_path,  sep, data_dir, data_transforms['val'],three_class=three_class)}
    ...
    if args.test:
        ...
        test_dataset = dataset.Final_dataset(args.test,  sep, data_dir, test_transforms,three_class=three_class)
    ...
    with open(args.out_file, "w") as f:
        ...
        # don't forget to reset stdout to original when done
